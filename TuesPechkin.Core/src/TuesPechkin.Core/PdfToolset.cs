...
namespace TuesPechkin
{
    public sealed class PdfToolset : MarshalByRefObject, IToolset
    {
        ...
        public void Load(IDeployment deployment = null)
        {
            ...
            //WinApiHelper.LoadLibrary(Path.Combine(Deployment.Path, "wkhtmltox.dll"));
            var ss=LibraryLoader.Instance.LoadLibrary(Path.Combine(Deployment.Path, "wkhtmltox"));
            Environment.SetEnvironmentVariable("LD_LIBRARY_PATH", Deployment.Path);
            WkhtmltoxBindings.wkhtmltopdf_init(0);
            Console.WriteLine("init wkhtmlttoxbindlings");
        ...
        public int SetObjectSetting(IntPtr setting, string name, string value)
        {
            ...
            var success = WkhtmltoxBindings.wkhtmltopdf_set_object_setting(setting,MarshalerCore.MarshalManagedToNative(name), MarshalerCore.MarshalManagedToNative(value));
